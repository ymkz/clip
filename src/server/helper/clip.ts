...
const fetchClipInfo = async (url: string): Promise<Response> => {
  ...
  const response = await fetch(url, { headers: { 'user-agent': UA } }).catch(
    (err) => {
      throw createFetchError({
        reason: 'client error when fetch clip info',
        url,
        cause: err,
      })
    }
  ...
  if (!response.ok) {
    throw createFetchError({
      reason: 'bad response from server when fetch clip info',
      url,
    })
  }
...
const fetchClipImage = async (url: string): Promise<ClipImage> => {
  const response = await fetch(url).catch((err) => {
    throw createFetchError({
      reason: 'client error when fetch clip image',
      url,
      cause: err,
    })
  })
  ...
  if (!response.ok) {
    throw createFetchError({
      reason: 'bad response from server when fetch clip image',
      url,
    })
  }
  ...
  const arrayBuffer = await response.arrayBuffer().catch((err) => {
    throw createFetchError({
      reason: 'clip image cannot parse as ArrayBuffer',
      url,
      cause: err,
    })
  })
